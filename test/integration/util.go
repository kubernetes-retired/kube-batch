...
const (
	pod_exec_time_key = "kube-batch.k8s.io/pod-exec-time"
...
type context struct {
	...
	ctrlPlane *integration.ControlPlane
...
func initTestContext() *context {
	...
	cxt.ctrlPlane, cxt.kubeclient, cxt.karclient = startCluster(cxt)
...
func createJob(
	...
	containers := createContainers(req, 0)
	return createJobWithOptions(context, name, min, rep, execTime, affinity, containers)
}

func createContainers(req v1.ResourceList, hostport int32) []v1.Container {
			...
			Template: v1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						arbv1.GroupNameAnnotationKey: jn,
						pod_exec_time_key:            execTime,
					},
				},
				Spec: v1.PodSpec{
