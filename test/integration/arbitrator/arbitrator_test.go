...
func prepareNamespace(cs *clientset.Clientset) error {
	...
	ns01, err := cs.CoreV1().Namespaces().Create(ns01)
	if err != nil {
		return fmt.Errorf("fail to create namespace ns01, %#v", err)
	}
	ns02, err = cs.CoreV1().Namespaces().Create(ns02)
	if err != nil {
...
func prepareNode(cs *clientset.Clientset) error {
	...
	baseNode, err := cs.CoreV1().Nodes().Create(baseNode)
	if err != nil {
...
func prepareResourceQuota(cs *clientset.Clientset) error {
	...
	rq01, err := cs.CoreV1().ResourceQuotas(rq01.Namespace).Create(rq01)
	if err != nil {
		return fmt.Errorf("fail to create quota rq01, %#v", err)
	}
	rq02, err = cs.CoreV1().ResourceQuotas(rq02.Namespace).Create(rq02)
	if err != nil {
...
func prepareCRD(config *restclient.Config) error {
	...
	resultList := apiv1.ResourceQuotaAllocatorList{}
	err = crdClient.Get().
		Resource(apiv1.ResourceQuotaAllocatorPlural).
		Do().Into(&resultList)
	if err != nil {
		return fmt.Errorf("fail to get crd list, %#v", err)
	}
	return nil
}
...
func TestArbitrator(t *testing.T) {
	...
	cpuQ01 := rq01.Spec.Hard["limits.cpu"]
	if v, _ := (&cpuQ01).AsInt64(); v != int64(5) {
		t.Fatalf("after scheduler, cpu is not 5 for rq01, %#v", rq01)
	}
	rq02, _ := cs.CoreV1().ResourceQuotas("ns02").Get("rq02", metav1.GetOptions{})
	cpuQ02 := rq02.Spec.Hard["limits.cpu"]
	if v, _ := (&cpuQ02).AsInt64(); v != int64(10) {
